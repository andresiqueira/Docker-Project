...
const FormCard = ({ emitter, ...rest }: FormCardProps) => {
  ...
  return (
    <form
      ...
      className="flex flex-col relative w-[24.375rem] lg:w-[33.125rem] bg-white border border-primary-border lg:rounded-[0.1875rem] rounded-3xl mt-6 mb-10 py-3 shadow-secondary-shadow"
      {...rest}
    >
      ...
      <textarea
        ...
        className="focus:outline-none w-full flex flex-row px-5 pt-2 pb-3 resize-none overflow-hidden"
        placeholder="Criar nota..."
      />
      {errors.textarea && <div className="text-red-500 m-auto mb-4">{errors.textarea}</div>}
